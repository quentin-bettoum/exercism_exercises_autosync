(module
  ...
  (func (export "compute") 
    ...
    ;; Loop while i < firstLength
    (loop $loop
      (if (i32.lt_u (local.get $i) (local.get $firstLength))
        (then
          ;; Load bytes and compare, then increment distance if they differ
          (local.set $distance
    ...
    ;; After loop exits (when i >= firstLength), return the distance
