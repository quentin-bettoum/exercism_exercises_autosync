defmodule ProteinTranslation do
  ...
  @doc """
  Given an RNA string, return a list of amino acid (composing a protein) specified by codons, in order.
  """
  ...
  def of_rna(rna) do
    ...
    |> Enum.reduce_while([], fn c, acc ->
      case of_codon(c) do
        ...
        {:ok, "STOP"} -> {:halt, acc}
        {:ok, amino_acid} -> {:cont, [amino_acid | acc]}
      end
    end)
    |> case do
      protein when is_list(protein) -> {:ok, Enum.reverse(protein)}
      _ -> {:error, "invalid RNA"}
    end
  end
